...
import org.utopia.stock.svr.dao.HoldMapper;
import org.utopia.stock.svr.entity.Hold;
import org.utopia.stock.svr.service.HoldService;
    ...
    public int importCsv(MultipartFile file) {
        ...
        lists.stream().forEach(info -> {
            mapper.insertSelective(
                    Hold.builder().id(idWorker.nextId()).date(info.get(0)).code(info.get(1)).build());
        });
