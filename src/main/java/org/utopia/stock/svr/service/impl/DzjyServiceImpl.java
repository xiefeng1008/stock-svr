...
import org.utopia.stock.svr.dao.DzjyMapper;
import org.utopia.stock.svr.entity.Dzjy;
import org.utopia.stock.svr.service.DzjyService;
...
@Service
public class DzjyServiceImpl implements DzjyService {

    @Resource
    private IdWorker idWorker;
    @Resource
    private DzjyMapper mapper;
    ...
    public int importCsv(MultipartFile file) {
        ...
        lists.stream().forEach(info -> {
            mapper.insertSelective(
                    Dzjy.builder().id(idWorker.nextId()).date(info.get(0)).code(info.get(1)).build());
        });
